...
@section('content')
    @component('components.breadcrumb')
        ...
        @endslot
        @slot('li_2')
            <a href="{{ route('permits') }}">@lang('translation.permits')</a>
        @endslot
        @slot('li_3')
            "#{{ $permit->number }}"
        @endslot
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <ul class="nav nav-tabs nav-tabs-custom nav-justified" role="tablist">
                        <li class="nav-item">
                            <a id="tab_link_profile" class="nav-link active" data-toggle="tab" href="#profile"
                               role="tab">
                                    ...
                                    <div class="row">
                                        <div class="col-lg-4 admin-form-item-widget">
                                            ...
                                            @endphp
                                                    ...
                                                    <x-form-select name="status" :items="{{$statusCB}}"
                                                           selected="{{ $permit->status ?? null }}"
                                                           :params="['label' => 'Status', 'required' => true]"></x-form-select>
                                        </div>
                                        <div class="col-lg-4">
                                            <x-form-text name="type" :
                                                         params="['label' => 'Type', 'iconClass' => 'fas fa-file','required' => true]">{{ $permit->type }}</x-form-text>
                                        </div>
                                        <div class="col-lg-4">
                                            <x-form-text name="number"
                                                         :params="['label' => 'Number', 'iconClass' => 'fas fa-folder']">{{ $permit->number }}</x-form-text>
                                        </div>
                                    </div>
                                </form>
                            </div>
                            ...
                            <div class="tab-pane" id="notes" role="tabpanel">
                                <div class="row">
                                    <div class="col-md-8 col-sm-6 mb20">
                                        <x-href-button id="add_note_button" class="btn-success"
                                                       data-route="{{ route('permit_note_add', ['permit' => $permit->id]) }}"
                                                       data-id="{{ $permit->id }}"
                                                       data-permit_name="{{ $permit->full_name }}"><i
                                                class="fas fa-plus"></i>@lang('translation.add')</x-href-button>
                                    </div>
                                ...
                                @foreach ($permit->notes as $notes)
                                    <div class="row">
                                        <div class="col-lg-10 col-md-9 col-sm-8 admin-form-item-widget">
                                            ...
                                            <x-form-show class="mh-100"
                                                         :params="['label' => 'none', 'iconClass' => 'fas fa-sticky-note']">{{ $notes->note ?? null }}</x-form-show>
                                        </div>
                                        <div class="col-lg-2 col-md-3 col-sm-4 admin-form-item-widget">
                                            <x-form-show class="mh-100"
                                                         :params="['label' => 'Fee', 'iconClass' => 'fas fa-dollar-sign']">{{ $notes->fee ?? '0.00' }}</x-form-show>
