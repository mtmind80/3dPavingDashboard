...
@section('content')
    @component('components.breadcrumb')
        @slot('title') @lang('translation.additionalCosts')@endslot
        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('workorder_additionalCost_store') }}" accept-charset="UTF-8" id="adminForm" class="admin-form">
                        @csrf
            ...
            @if (!empty($additionalCosts))
                <div class="card">
                    <div class="card-body">
                        <h5 class="mb-4">@lang('translation.additionalCosts')</h5>
                        <table class="list-table table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                            ...
                            <tbody>
                                @foreach ($additionalCosts as $additionalCost)
                                    <tr {{ !empty($additionalCost->status->color) ? ' style=background-color:#'.$additionalCost->status->color.' ' : '' }}data-id="{{ $additionalCost->id }}">
                                        <td class="tc">{{ $additionalCost->additionalCost->name ?? null }}</td>
                                        <td class="tc">{{ \App\Helpers\Currency::format($additionalCost->cost) }}</td>
                                        <td class="tc">{{ $additionalCost->description }}</td>
                                        <td class="centered">
                                            <form action="{{ route('workorder_additionalCost_destroy',['workorder_additionalCost_id' => $additionalCost->id]) }}" method="post">
                                                @csrf
                        ...
                        <x-paginator
                            :collection="$additionalCosts"
                            route-name="workorder_additionalCost_list"
                            :params="[
