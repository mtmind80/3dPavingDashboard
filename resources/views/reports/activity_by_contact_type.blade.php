...
@section('content')
    @component('components.breadcrumb')
        ...
        @slot('li_3') @lang('translation.activitybycontacttypereport') @endslot
    @endcomponent
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-sm-12 mb0">
                            ...
                            <form action="{{ route('activity_by_contact_type_report_export') }}" method="post" id="reportForm">
                                @csrf
                                ...
                                <div class="row">
                                    <div class="col-lg-3 col-md-12 admin-form-item-widget">
                                        <x-form-date-range-picker
                                            ...
                                            class=""
                                            :params="[
                                                'id' => 'activity_by_contact_type_date_range_picker',
                    ...
                    <div id="report_header" class="row hidden">
                        <div class="col-sm-12 mb20">
                            <div class="d-flex flex-row flex-xxl-column justify-content-between align-items-center">
                                <div class="">
                                    <h5>@lang('translation.activitybycontacttypereport')</h5>
                                </div>
                                <div class="">
                    ...
                    <table id="report_table" class="hidden list-table table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        <thead>
                        <tr>
                            <th class="tc w400">@lang('translation.activitybycontacttypereport')</th>
...
@section('page-js')
    <script>
        $(document).ready(function(){
            var dateRangePicker = $('#activity_by_contact_type_date_range_picker');
            ...
            $('#create_report').click(function (){
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: "{{ route('activity_by_contact_type_report_ajax_view') }}",
                        ...
                        if (response.success) {
                            ...
                            $.each(response.rows, function (sales_person_name, row){
                                html +=
                                    ...
                                    '</tr>' +
                                    '<tr class="tr-header">' +
                                        '<td class="tc">'+ "{{ __('translation.contacttype') }}" +'</td>' +
                                        '<td class="tc">'+ "{{ __('translation.proposals') }}" +'</td>' +
                                        '<td class="tc">'+ "{{ __('translation.cost') }}" +'</td>' +
                                        '<td class="tc">'+ "{{ __('translation.work_orders') }}" +'</td>' +
                                        '<td class="tc">'+ "{{ __('translation.cost') }}" +'</td>' +
                                    '</tr>';

                                    $.each(row.contact_types, function(contact_type_name, data){
                                        html +=
                                            '<tr class="">' +
                                                '<td class="tc">'+ contact_type_name +'</td>' +
                                                '<td class="tc">'+ data.proposal_total +'</td>' +
                                                '<td class="tc">'+ data.proposal_cost +'</td>' +
                                                '<td class="tc">'+ data.work_order_total +'</td>' +
                                                '<td class="tc">'+ data.work_order_cost +'</td>' +
                                            '</tr>';
                                    });
                                    ...
                                    html +=
                                        '<tr class="tr-footer">' +
                                            ...
                                            '<td class="tc">&nbsp;</td>' +
                                            '<td class="tc">'+ row.global_formatted_proposal_cost +'</td>' +
                                            '<td class="tc">&nbsp;</td>' +
                                            '<td class="tc">'+ row.global_formatted_work_order_cost +'</td>' +
                                        '</tr>';
                            });
